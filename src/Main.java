...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            market.addStock(new Stock("JOON", setup.joonPrice));
            market.addStock(new Stock("HXMZA", setup.hxmzaPrice));

            for (int i = 1; i <= 6; i++) {
                market.addTrader(new RetailTrader("Retail" + i,5000));
            }
            
            for (int i = 1; i <= 2; i++) {
                market.addTrader(new InstitutionalTrader("Institution" + i, 100000));
            }
            ...
            MarketPanel marketPanel = new MarketPanel(market);

            JScrollPane scrollPane = new JScrollPane(marketPanel);

            scrollPane.getViewport().setBackground(new Color(13, 16, 20));

            JButton startButton = new JButton("Start");
            JButton pauseButton = new JButton("Pause");
            JButton stepButton = new JButton("Go one Step");

            JLabel tickLabel = new JLabel("Tick: 0");

            tickLabel.setForeground(new Color(210, 216, 224));

            JPanel controls = new JPanel(new FlowLayout(FlowLayout.LEFT));

            controls.setBackground(new Color(17, 21, 26));
            ...
            tradesPanelHolder[0] = new TradesPanel(market, repaintAll);

            TradesPanel tradesPanel = tradesPanelHolder[0];

            Timer timer = new Timer(500, e -> runOneTick(market, repaintAll));
            ...
            stepButton.addActionListener(e -> {
                if (!timer.isRunning()) {
                    runOneTick(market, repaintAll);
                }
            });

            frame.add(controls, BorderLayout.NORTH);
            frame.add(scrollPane, BorderLayout.CENTER);
            frame.add(tradesPanel, BorderLayout.EAST);
            frame.setVisible(true);
        });
    }

    private static void runOneTick(Market market, Runnable repaintAll) {
        market.update();
        repaintAll.run();
    }
    ...
    private static Setup askSetup() {
        ...
        while (true) {
            int result =
                    JOptionPane.showConfirmDialog(null, panel, "Set up the market", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
            ...
            try {
                ...
                JOptionPane.showMessageDialog(null, e.getMessage(), "Invalid setup", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
